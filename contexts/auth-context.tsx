...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signUp = async (
    ...
    // All new users start as "pending" until admin approves
    await setDoc(doc(db, "users", newUser.uid), {
      email,
      displayName,
      phone,
      role: "landlord",
      subscriptionStatus: "pending", // Pending until admin approves
  ...
  const isDemoUser = userData?.subscriptionStatus === "demo" || userData?.subscriptionStatus === "expired";
  
  // Check if user can access dashboard
  const canAccessDashboard = isAdmin || userData?.subscriptionStatus === "active" || isDemoMode;
  ...
  const getUserStatusMessage = (): string | null => {
    ...
    switch (userData.subscriptionStatus) {
      case "pending":
        return "আপনার পেমেন্ট ভেরিফিকেশনের অপেক্ষায় আছে। Admin approve করলে আপনি dashboard access পাবেন।";
      case "banned":
      ...
      case "expired":
        return "আপনার সাবস্ক্রিপশন মেয়াদ শেষ হয়ে গেছে। রিনিউ করুন।";
      default:
        return null;
    }
  };
